...
4. In Authentication settings, allow email/password sign-up, turn off email confirmation for this invite-only prototype, and add the deployed site URL to the allowed redirect URLs.
...
The free Supabase project can pause after a week of inactivity. Email addresses and phone numbers are not verified in this no-SMS setup. Only share invite links with the intended friend. For verified email, configure a custom SMTP provider; phone OTP requires an SMS provider and may cost extra.